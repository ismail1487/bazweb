...
// #region Silinen Değer Dizeleri
const SilinenSpotlar = [];
...
function TekrarliSpotEkle() {
    ...
    console.log("Tekrarlı Spot Id: " + tekrarliSpotId);
    let spotHtml = '<div class="row" data-id="' + tekrarliSpotId +'">\
        <div class="col-md-6">\
...
function SpotSil(button) {
    ...
    console.log("Silinen Spot veri:" + SpotMetni);
...
function TekrarliMedyaEkle() {
    ...
    console.log("Tekrarlı medya Id: " + tekrarliMedyaId);
...
function IcerikVeriGetir() {
    console.log("Icerik veri getir çalıştırıldı");
    $.ajax({
        type: "GET",
        url: "/IcerikGiris/IcerikVeriGetir/" + URlIdCek(), //Url'den Çekilen Id
        dataType: "json",
        success: function (result) {
            ...
            if (result.isSuccess && result.value && result.value.length > 0) {
                ...
                console.log("Spotlar:", veri.icerikSpot);
                console.log("Medyalar:", veri.medyalar);
                IcerikVeriDoldur(veri);
            }
            else {
...
//function Select2DegerKullan(id, value) {
//    if (!value) return; // null, undefined veya boşsa geç
//    let $select = $("#" + id);

//    // Eğer option yoksa, ekle
//    if ($select.find("option[value='" + value + "']").length === 0) {
//        $select.append(new Option(value, value, true, true)); // text ve value aynı, seçili
//    }

//    $select.val(value).trigger("change");
//}

function IcerikVeriDoldur(veri) {
    //Kategoriler
    console.log(veri.paramAltKategoriID);
    console.log(veri.paramAnaKategoriID);
    console.log(veri.paramIcerikKategoriID);
        $("#AnaKategori").val(veri.paramAnaKategoriID.toString()).trigger("change");
        //Kategori için de aynı mantık
        setTimeout(() => {
            $("#Kategori").val(veri.paramIcerikKategoriID.toString()).trigger("change");
    ...
    // Başlık
    $("#Baslik_1").val(veri.icerikBaslik);

    // Quill
    quill.root.innerHTML = veri.icerikTamMetin;

    // Tarihler
    $("#yayinlanmaTarihi").val(veri.icerikYayinlanmaZamani?.split("T")[0] ?? "");
    $("#KaldirilmaTarihi").val(veri.icerikKaldirilmaZamani?.split("T")[0] ?? "");
    ...
    $("#tekrarliSpot").empty();
    tekrarliSpotId = -1;
    if (veri.icerikSpot && veri.icerikSpot.length > 0) {
        veri.icerikSpot.forEach(spot => {
            SpotEkle();
            $("#Spot" + tekrarliSpotId).val(spot.spotMetni);
        });
    } else {
        SpotEkle(); // Boşsa 1 tane aç
    }
    ...
    if (veri.medyalar && veri.medyalar.length > 0) {
        veri.medyalar.forEach(m => {
            ...
            // Önceki medya varsa, preview gösterme alanı
            if (m.medyaUrl) {
                // Tek URL ise
            ...
            // Medya ID'yi hidden input ya da data attribute olarak(kayıt için)
...
function MedyaSil(button) {
    ...
    if (dataId !== undefined) {
        // Dosya adını veya başka bilgileri al
        const dosyaAdi = $("#medyaAdi").val();

        // Silinen medya bilgisi olarak kaydet
...
function HataUyarilari() {
    ...
    for (let i = 0; i < medyaCount; i++) {
        ...
        const dosyaPreview = document.getElementById(`dosyaPreview${i}`);
        ...
        // Eğer dosya seçilmemişse ve preview'de data-media-id yoksa hata ver
        if (!dosyaEkle.value && (!dosyaPreview || !dosyaPreview.getAttribute('data-media-id'))) {
...
function VerileriModeleBas() {
    ...
    IcerikModelVM["TabloID"] = URlIdCek();
    ...
    $('#tekrarliSpot input[id^="Spot"]').each(function () {
        ...
        IcerikModelVM["SilinenSpotlar"] = SilinenSpotlar;
    });
...
function IcerikModeliGuncelle() {
    var model = VerileriModeleBas();
    $.ajax({
        type: "POST",
        url: "/IcerikGiris/IcerikGuncelle",
        dataType: "json",
