...
(function ($) {
    $.fn.EasyTree = function (options) {
        var defaults = {
            ...
            addable: false,
            i18n: {
                deleteNull: 'Lütfen silinecek öğeyi seçin.',
                deleteConfirmation: 'Silmek istediğinize emin misiniz?？',
                confirmButtonLabel: 'Onayla',
                editNull: 'Onayla',
                editMultiple: 'Bir seferde yalnızca bir öğeyi düzenleyebilirsiniz.',
                addMultiple: 'Lütfen eklemek için birini seçin.',
                collapseTip: 'Şubeyi daralt.',
                expandTip: 'Şubeyi genişlet.',
                selectTip: 'Seç',
                unselectTip: 'Seçimi iptal et.',
                editTip: 'Seçimi iptal et',
                addTip: 'Ekle',
                deleteTip: 'Sil',
        ...
        this.each(function () {
            ...
            var onEditable = function () {
                $('#btns').html('');
                $('#btns').html(
                    '<a href="javascript:;" id="btnupdate" class="btn add-buttons-nofloat  mx-1">' + siteLang.Guncelle2 + '</a>');
                $('#btns').append('<a href="javascript:;" id="btnRemove" class="btn cancel-button mx-1">' + siteLang.Delete + '</a>');
                $('#btns').append('<a href="javascript:;" id="btnCancel" class="btn cancel-button-float">' + siteLang.Vazgec + '</a>');
                var item = getSellectedTree();
                $("#Tanim").val(item.tanim);
                if (item.koordinat != null && item.koordinat != undefined) {
                    var coords = item.koordinat.split(',');
                    $('#KoordinatX').val(coords[0]);
                    $('#KoordinatY').val(coords[1]);
                } else {
                    $('#KoordinatX').val('');
                    $('#KoordinatY').val('');
                }
                getKurumOrganizasyonSelectedForLevel(item.tabloId, function () {
                    $("#select-organizasyon-birimi").select2().val(item.ustId).trigger("change");
                });
            }
...
function getSellectedTree() {
    var item = $('.easy-tree').find('li.li_selected');
    var result = { tabloId: item.data('tabloid'), tipId: item.data('tipid'), ustId: item.data('ustid'), koordinat: item.data('coords'), tanim: $('.easy-tree').find('li.li_selected  a').html() };
    return result;
}
function offEditable() {
    $('#btns').html('');
    $('#btns').html("<a href='javascript:;' id='btnsave' class='btn add-buttons'>" + siteLang.Kaydet2 + "</a>");
    $("#Tanim").val('');
    $('#KoordinatX').val('');
    $('#KoordinatY').val('');
    $('.li_selected').removeClass('li_selected');
    // $("#select-organizasyon-birimi").select2().val(0).trigger("change");
    $('#select2-select-organizasyon-birimi-container').removeAttr(siteLang.Choose);
    $('#select2-select-organizasyon-birimi-container').html(siteLang.Choose);
}