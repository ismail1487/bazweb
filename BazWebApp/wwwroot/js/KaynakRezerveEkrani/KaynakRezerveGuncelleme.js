...
function UrunVeriGetir() {
    $.ajax({
        type: "GET",
        url: "/KaynakRezerve/KaynakRezerveVeriGetir/" + URlIdCek(), //Url'den Çekilen Id
        dataType: "json",
...
function UrunVeriDoldur(veri) {
    //Tip
    var KaynakRezerveTanimVM = veri.kaynakRezerveTanimVM;
    var KaynakTanimVM = veri.kaynakTanimVM;
    var kapasiteVM = veri.kapsiteVM;
    $("#kaynakTipi").val(KaynakTanimVM.paramKaynakTipiID.toString()).trigger("change");

    // Başlık
    $("#kaynakTanim").val(KaynakTanimVM.kaynakAdi);

    //Açıklama
    $("#kaynakAciklama").val(KaynakTanimVM.kaynakAciklama);

    //Uygun Gün Aralığı
    if (KaynakRezerveTanimVM.uygunGunTipleri == 1) $("#HaftaIci").prop("checked", true);
    if (KaynakRezerveTanimVM.uygunGunTipleri == 2) $("#HaftaSonu").prop("checked", true);
    if (KaynakRezerveTanimVM.uygunGunTipleri == 3) $("#TamHafta").prop("checked", true);
    //Tarihler
    $("#rezerveSaatBaslangic").val(KaynakRezerveTanimVM.rezerveSaatBaslangicDegeri);
    $("#rezerveSaatBitis").val(KaynakRezerveTanimVM.rezerveSaatBitisDegeri);

    //Kapasite
    $("#kaynakKapasite").val(kapasiteVM[0].kapasaite);

    // Medyalar
    $("#tekrarliMedya").empty();
    tekrarliMedyaId = -1;
    if (veri.medyaVM && veri.medyaVM.length > 0) {

        veri.medyaVM.forEach(m => {
            ...
            $("#dosyaPreview" + id).attr("data-media-id", m.resimID);
        });
    } else {
        MedyaEkle();  // boşsa en az 1 tane ekle
    }

    $("#tekrarliIstisna").empty();
    tekrarliIstisnaId = -1;
    if (veri.kaynakIstisnalariVM && veri.kaynakIstisnalariVM.length > 0) {

        veri.kaynakIstisnalariVM.forEach(m => {
            IstisnaEkle();  // İlk medya bloğu ekle
            let id = tekrarliIstisnaId;
            // medya değerlerini doldur
            $("#istisnaTarihBaslangic" + id).val(m.istisnaTarihBaslangicDegeri.split('T')[0]);
            $("#istisnaTarihBitis" + id).val(m.istisnaTarihtBitisDegeri.split('T')[0]);

            $("#molaSaatBaslangic" + id).val(m.istisnaSaatBaslangicDegeri);

            $("#molaSaatBitis" + id).val(m.istisnaSaatBitisDegeri);

        });
    } else {
        IstisnaEkle();  // boşsa en az 1 tane ekle
    }
}
function KaynakGuncelle() {
    ...
    if (hataVar) {
        ...
        KaynakRezerveGuncellet();
    }
    Errors = false;
}

function KaynakRezerveGuncellet() {
    var model = VerileriModeleBas();
    console.log(JSON.stringify(model, null, 2));
    $.ajax({
        type: "POST",
        async: false,
        url: "/KaynakRezerve/RezerveKaynakGuncelle",
        dataType: "JSON",
...
function VerileriModeleBas() {
    ...
    KaynakRezerveVM["TabloID"] = URlIdCek();
    // KAYNAK TANIM
    ...
    KaynakRezerveVM["KaynakRezerveTanimVM"] = {
        RezerveSaatBaslangicDegeri: $("#rezerveSaatBaslangic").val(),
        RezerveSaatBitisDegeri: $("#rezerveSaatBitis").val(),
    ...
    $('#tekrarliIstisna > div').each(function () {
        ...
        // Sadece tüm alanlar null değilse ekle
        if (istisna.IstisnaSaatBaslangicDegeri || istisna.IstisnaSaatBitisDegeri ||
            istisna.IstisnaTarihBaslangicDegeri || istisna.IstisnaTarihtBitisDegeri) {
            KaynakRezerveVM["KaynakIstisnalariVM"].push(istisna);
        }
    });
...
function IstisnaIdleriGuncelle() {
    $("#tekrarliIstisna .row").each(function (index) {
        $(this).find('input.form-control').each(function () {
            // Örn: "molaSaatiBaslangic0" → "molaSaatiBaslangic"
            let baseId = this.id.replace(/[0-9]+$/, "");
            // Yeni id ata
            this.id = baseId + index;
        });
    });
}