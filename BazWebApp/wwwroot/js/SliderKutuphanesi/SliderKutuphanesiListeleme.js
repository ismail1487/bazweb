...
function GuncelemeSayfasinaGit(urlId) {
    window.location.href = `/Slider/SliderGuncelleme/${urlId}`;
}

function EklemeSayfasi() {
    window.location.href = `/Slider/SliderGiris/`;
}
function SliderListesiGetir() {
    $.ajax({
        type: "GET",
        url: "/Slider/SliderListesi",
        dataType: "json",
        data: null,
        success: function (result) {
            if (result.isSuccess == true) {
                console.log(result.value);
                $("#SliderListesiTable").DataTable().destroy();
                var satirlar = "";
                console.log(result.value);
                data = result.value;
                for (var i of data) {
                    satirlar += "<tr >\
                    <td >" + i.sliderGorselTanim + "</td>\
                    <td >" + i.sliderGorselAciklama + "</td>\
                    <td >" + i.sliderResimlerMedyalarVM.length + "</td>\
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    Swal.fire({
                        title: "Silindi!",
                        text: "Slider Listesinden silindi",
                        icon: "success"
                    });
