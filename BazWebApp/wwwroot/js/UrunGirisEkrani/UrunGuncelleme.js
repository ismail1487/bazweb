        ...
        <div class="col-md-4" >\
            ...
            <div class="col-md-4">\
                <div class="form-group">\
                    ...
                    <label for="blokmedyaustmetin" id="blokmedyaustmetinHata'+ tekarliBlokMedyaId + '" class="validationMessage">Açıklama</label>\
                </div>\
            </div>\
            <div class="col-md-4">\
                <div class="form-group">\
                    <label class="Lbl">Dosya Ekle</label>\
                    <input type="file" id="blokdosyaEkle' + tekarliBlokMedyaId + '" class="form-control" style="width: 100%;"/>\
...
function UrunModeliGuncelle() {
    var model = VerileriModeleBas(); 
    const loader = document.getElementById("loader");
    $.ajax({
        type: "POST",
        url: "/UrunGiris/UrunGuncelle", 
        dataType: "json",
...
function UrunVeriGetir() {
    console.log("Icerik veri getir çalıştırıldı");
    $.ajax({
        type: "GET",
        url: "/UrunGiris/UrunVeriGetir/" + URlIdCek(), //Url'den Çekilen Id
        dataType: "json",
        success: function (result) {
...
function UrunVeriDoldur(veri) {
    //Kategoriler
    console.log(veri.paramAltKategoriID);
    console.log(veri.paramAnaKategoriID);
    console.log(veri.paramUrunKategoriID);
    $("#AnaKategori").val(veri.paramAnaKategoriID.toString()).trigger("change");
    setTimeout(() => {
        $("#Kategori").val(veri.paramUrunKategoriID.toString()).trigger("change");

        setTimeout(() => {
            $("#Alt-Kategori").val(veri.paramAltKategoriID.toString()).trigger("change");
        }, 300);
    }, 300);

    // Başlık
    $("#UrunAdi_1").val(veri.urunAdi);

    // Quill
    quill.root.innerHTML = veri.urunAciklama || "";

    // Taslak
    $("#TaslakMiCheckbox").prop("checked", veri.taslakMi === true || veri.taslakMi === 1);

    if (veri.urunFiyatVM && veri.urunFiyatVM.length > 0) {
        veri.urunFiyatVM.forEach(u => {
            $("#urunFiyat").val(u.urunFiyat);
            $('#urunParabirimi').val(u.paraBirimiID).trigger("change");
        });
    }
    else { console.log("Hata Veri Fiyat Hatası"); }

    if (veri.urunParametre && veri.urunParametre.length > 0) {
        const u = veri.urunParametre[0];
        console.log(u);

        $("#HizKategori").val(u.urunHiz);
        $("#HacimKategori").val(u.urunHacim);
        $("#OlcumKategori").val(u.urunOlcum);

        // Birim seçimleri - bunlar sabit select listesi gibi görünüyor
        $('#HizBirim').val(u.urunHizBirimID).trigger("change");
        $('#HacimBirim').val(u.urunHacimBirimID).trigger("change");
        $('#OlcumBirim').val(u.urunOlcumBirimID).trigger("change");

        // Marka seçilince model yükleniyorsa bunu beklememiz lazım
        $("#MarkaKategori").val(u.urunMarkaID.toString()).trigger("change");

        // Marka değişince AJAX ile model ve seri yükleniyorsa bu işlemler gecikmeli olmalı
        setTimeout(() => {
            $("#ModelKategori").val(u.urunModelID.toString()).trigger("change");

            setTimeout(() => {
                $("#SeriKategori").val(u.urunSeriID.toString()).trigger("change");
            }, 500); // Seri için biraz daha bekleyelim
        }, 500); // Model için zaman tanıyalım
    }
    else {
        console.log("Hata Veri Param Hatası");
    }

    if (veri.icerikBloklar && veri.icerikBloklar.length > 0) {
        const u = veri.icerikBloklar[0];
        console.log(u);

        $("#IcerikBlokTanim").val(u.icerikBlokTanim);

        $('#IcerikKutuphanesi').val(u.icerikKutuphanesiID).trigger("change");

        $("#IcerikBlokKategori").val(u.paramBlokKategoriID.toString()).trigger("change");

        setTimeout(() => {
            $("#IcerikBlokAlt-Kategori").val(u.paramBlokAltKategoriID.toString()).trigger("change");
        }, 500);
    }
    else {
        console.log("Hata Veri Param Hatası");
    ...
    $("#tekrarliMedya").empty();
    tekrarliMedyaId = -1;
    if (veri.medyalar && veri.medyalar.length > 0) {

        veri.medyalar.forEach(m => {
            MedyaEkle();  // İlk medya bloğu ekle
            let id = tekrarliMedyaId;
            // medya değerlerini doldur
            $("#medyaTipi" + id).val(m.paramMedyaTipiID).trigger("change");
            $("#medyaAdi" + id).val(m.medyaAdi);
            $("#gosterimAdi" + id).val(m.gosterimAdi);
            $("#sira" + id).val(m.siraNo);
            $("#altMetin" + id).val(m.altMetin);
        ...
        MedyaEkle();  // boşsa en az 1 tane ekle
    }

    // İkinci medya alanı için (blok medya)
    $("#tekrarliMedyaBlok").empty();
    tekarliBlokMedyaId = -1;
    if (veri.icerikMedyalar && veri.icerikMedyalar.length > 0) {
        veri.icerikMedyalar.forEach(m => {
            BlokMedyaEkle();  // Bu fonksiyon olmalı, MedyaEkle değil
            let id = tekarliBlokMedyaId;
            $("#blokmedyaTipi" + id).val(m.paramMedyaTipiID).trigger("change");
            $("#bloksira" + id).val(m.siraNo);
            $("#blokmedyaustmetin" + id).val(m.ustMetin);  // ustMetin kullan
            $("#blokmedyaAciklama" + id).val(m.medyaAciklama);

            if (m.medyaUrl) {
                let html = `<img style="width: 100px; margin-right: 5px;" src="http://localhost:51305${m.medyaUrl}" />`;
                $("#blokdosyaPreview" + id).html(html);
                $("#blokdosyaEkle" + id).removeAttr("required");
            } else {
                $("#blokdosyaEkle" + id).attr("required", "required");
            }
            $("#blokdosyaPreview" + id).attr("data-media-id", m.medyaID);
        });
    } else {
        BlokMedyaEkle();  // boşsa en az 1 tane ekle
    }
}
