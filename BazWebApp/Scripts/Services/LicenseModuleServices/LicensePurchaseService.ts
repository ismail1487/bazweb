...
class LicensePurchaseService extends kendo.data.ObservableObject {
    ...
    data = new KurumLisansOdemeModel({
        TabloID: 0,
        LisansId: 0,
        Name: "",
        KurumID: 0,
    ...
    zamanData = new LisansZamanModel({
        GecerliOlduguGun: 0,
        LisansId: 0,
        TabloID: 0,
        Name: 0
    ...
    kurumList: DropDownModel[];
    lisansList: DropDownModel[];
    lisansZamanList: DropDownModel[];
    lisansZamanFiyatList: DropDownModel[];
    lisansZamanKurList: DropDownModel[];
    paraBirimList: { id: number, title: string }[];
    zamanList: { id: number, gun: string, kur: string, fiyat: string, token: string, lisansPaketiKisiSayisi: number }[];
    KurumGetir() {
        this.kurumList = [];
    ...
    KurumLisansVeriGetir(id?: number) {
        var conf = AjaxConfiguration.getDafault();
    ...
    FreeLisansSatinAl() {
        ...
        conf.success = function (result) {
            if (result.isSuccess) {
                alertim.toast("İşleminiz başarılı. Giriş sayfasına yönlendiriliyorsunuz.",
                    "success",
                    function () {
                        $('#logout-btn').click();
                    });
...
var JSDateToStringCSDate: any;
var GetURLParameter: any;
var errorhandler: any;
var alertim: any;
var lisansSonKullanimTarihi: any;