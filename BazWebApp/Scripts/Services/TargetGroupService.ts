class TargetGroupService extends kendo.data.ObservableObject {
    ...
    getData(e) {
        var item: any = targetGroupService.fields.filter(function (value: any) { return value.name == e.MemberName; })[0];
    ...
    removeItem(item) {
        console.log(item)
        var filtre: any = targetGroupService.filter;
        filtre.remove(item);
    }
    fields: [];
    filter: TargetGroupModel[];
    targerGroupTypes = ["Kurum", "Kişi"];
...
var targetGroupService: TargetGroupService;
class TargetGroupModel {
    LocigalOperator?: string;
    Operator?: string;
    MemberName?: string;
    Value?: string;
    FilterType: string;
    FieldType?: string;
    InputType?: string;
    ParametreTableName?: string;
    EkParametreId?: number;
    FieldName?: string;
    filters?: TargetGroupModel[];
    TableName?: string;
    RowID?: string;
}
